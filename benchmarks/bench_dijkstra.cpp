...
int main(int argc, char** argv) {
    ...
    // ===== קבלת נתיב RB: CLI -> ENV -> חיפוש לאחור =====
    std::string rb_path = cli_get_arg(argc, argv, "--rb=");
    ...
    // אם קיבלנו --rb אבל לא קיים, ניפול לפולבאקים
    if (!rb_path.empty() && !exists_file(rb_path)) {
        LOG_INFO() << "[WARN] Given --rb=" << rb_path << " not found. Falling back.";
        rb_path.clear();
    }
    ...
    if (rb_path.empty()) {
        if (const char* env = std::getenv("SSSP_RB_PATH")) {
            ...
            if (exists_file(p)) {
                ...
                LOG_INFO() << "[WARN] SSSP_RB_PATH points to missing file: " << p.string();
            }
        }
    }

    // חיפוש לאחור עד 8 רמות ל-"data/patents_main.rb"
    if (rb_path.empty()) {
        namespace fs = std::filesystem;
        try { LOG_INFO() << "CWD=" << fs::current_path().string(); }
        catch (...) {}

        const fs::path def = fs::path(SSSP_DEFAULT_RB); // "data/patents_main.rb"
        fs::path up; fs::path found;
        for (int k = 0; k <= 8; ++k) {
            fs::path cand = up / def;    // "", "../data/...", "../../data/..." ...
            if (exists_file(cand)) {
        ...
        if (found.empty()) {
            LOG_ERROR() << "RB file not found. Tried default " << def.string()
                << " up to 8 parents from CWD. Use --rb=... or set SSSP_RB_PATH.";
            return 1;
        }
