...
int main(int argc, char** argv) {
    ...
    // קיבלנו --rb= ...? אם כן - השתמש רק אם קיים
    if (!rb_path.empty() && !exists_file(rb_path)) {
        LOG_INFO() << "[WARN] Given --rb=" << rb_path << " not found. Falling back to defaults.";
        rb_path.clear();
    }
    ...
    if (rb_path.empty()) {
        ...
        // עזרה לאבחון: ספריית העבודה בפועל
        try { LOG_INFO() << "CWD=" << fs::current_path().string(); }
        catch (...) {}

        // חיפוש דיפולטי כמה רמות אחורה:
        // def = "data/patents_main.rb" (מוגדר ב-config_paths.hpp)
        const fs::path def = fs::path(SSSP_DEFAULT_RB);

        fs::path up;       // "", "..", "../..", ...
        fs::path found;    // אם מצאנו – נתמלא

        for (int k = 0; k <= 8; ++k) {          // עד 8 רמות למעלה
            fs::path cand = up / def;          // up="" -> "./data/...", אח"כ ../data/...
            if (exists_file(cand)) {
                try { found = fs::canonical(cand); }
                catch (...) { found = cand; }  // אם canonical נכשל, נשתמש בנתיב היחסי שמצאנו
                break;
            }
            up /= ".."; // עלה רמה
        }
        ...
        if (found.empty()) {
            LOG_ERROR() << "RB file not found. Tried up to 8 parents from CWD for " << def.string();
            return 1;
        }
        rb_path = found.string();
    }
    ...
    // שורת סיכום ידידותית ללוג הקיים
    LOG_INFO() << "[BMSSP] N=" << times_bmssp.size()
        << " mean=" << st_bmssp.mean
        << " median=" << st_bmssp.median
        << " p95=" << st_bmssp.p95 << " s";
